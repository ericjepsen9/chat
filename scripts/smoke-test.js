...
(async () => {
  ...
  const first = await j(`/api/conversations/${conv.conversation.id}/messages`, {
    method: 'POST',
    body: JSON.stringify({ senderId: login.user.id, type: 'text', text: 'smoke', clientMessageId: 'smoke-fixed-id' }),
  });
  const second = await j(`/api/conversations/${conv.conversation.id}/messages`, {
    method: 'POST',
    body: JSON.stringify({ senderId: login.user.id, type: 'text', text: 'smoke-dup', clientMessageId: 'smoke-fixed-id' }),
  });
