...
const state = { 
  ...
  systemMessages: [],
  adminDashboard: null,
  buyerOrders: [], sellerOrders: [], sellerProducts: [], selectedOrderDetail: null, selectedOrderRole: 'buyer', selectedProductDetail: null, broadcastDrafts: [], tradePickerResolver: null,
...
function buildMessageChunk(msg, prevCreatedAt = 0) {
  ...
  if (msg.type === 'system') {
    ...
    if (msg.type === 'audio') {
      ...
      const isContactCard = c.cardType === '名片';
      if (isContactCard) {
      ...
      if (isContactCard && c.userId) {
        ...
        card.addEventListener('click', (e) => {
          e.stopPropagation();
          window.openUserProfile(c.userId, c.title || '用户');
        });
      }
